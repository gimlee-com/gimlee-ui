...
- **React Event Delegation**: The `Modal` wrapper component uses **React Portal** (`createPortal`) to render modals at the `#root` element, keeping them within React's event system while ensuring proper full-viewport overlay positioning. UIkit's `container` option is always set to `false` internally to prevent UIkit from moving DOM nodes (which conflicts with React's reconciliation and causes `removeChild` crashes on unmount). When using `useUIKit('modal', ...)` directly (without the `<Modal>` wrapper), always set `container: false` and wrap the JSX in `createPortal(jsx, document.getElementById('root') || document.body)` to achieve the same effect. **Never use UIkit's `container: '#root'`** — it physically moves DOM nodes via `appendChild`, which React cannot track.
...
#### **X. Adaptive Density & "Breathing Room" (ADR-003)**
How content compresses on narrower screens is governed by the **Liquid Crystal Adaptive Density System** (see `docs/adr/003-liquid-crystal-adaptive-density-system.md`), not by per-component media queries. The goal is maximum information density on phones (e.g. 2 ad cards per row) without sacrificing readability or accessibility.
- **Token-Driven, Not Per-Component**: Spacing/typography adapt via global CSS custom properties defined in `src/styles/main.scss` (`--density-pad`, `--density-stack`, `--density-gap`, `--fluid-text-*`, `--tap-min`). Components *consume* these tokens; they never re-implement responsive spacing locally.
- **Three Density Tiers**: `comfortable` (desktop) → `compact` (tablet, `< $breakpoint-medium`) → `dense` (phones, `< $breakpoint-small`). Tokens step down per tier via UIkit `$breakpoint-*-max` media queries — never hardcoded pixel breakpoints.
- **Gutter ("Breathing Room") = UIkit Variables**: The grid gap is the canonical `--density-gap` token and **must map to UIkit's native gutter scale**, never raw pixels: `$global-medium-gutter` (40px, desktop) → `$global-gutter` (30px, tablet) → `$global-small-gutter` (15px, phone). Generous gutters on desktop prevent dense grids from overwhelming the user; tight gutters on mobile let two cards fit per row. A single UIkit gutter retune then cascades everywhere.
- **`Grid gap="adaptive"`**: Listing surfaces apply the adaptive gutter through the `Grid` wrapper's `gap="adaptive"` prop (which emits the `.uk-grid-adaptive` utility consuming `--density-gap`). Do **not** hand-roll gutter pixels on listing pages.
- **Literals Only When UIkit Can't Express It**: Per §19, the *deliberately* tighter card-interior tokens (`--density-pad` 16/12/10, `--density-stack` 12/8/6) stay explicit because UIkit's smallest spacing step (15px) is already too generous for a 2-up mobile card. The rule is *"don't hardcode a value a UIkit variable already expresses"* — not *"never use a literal."* Always document why a literal survives.
- **Continuous Fluid Type**: Font sizes use `clamp()` (`--fluid-text-*`) for smooth interpolation between a mobile floor (~360px) and desktop ceiling (~1200px), avoiding breakpoint "jumps." Floors (12px readable / 14px interactive) and `--tap-min: 44px` are non-negotiable accessibility limits.
- **Progressive Ornament Recede**: When a surface goes dense, ornament recedes in priority order (decorative footers/dividers → secondary meta icons → tertiary lines), but title, price, location label, and tap targets are never removed.
...
24. **React-Compatible Modals**: The `Modal` wrapper uses React Portal to render at `#root`, while always setting UIkit's `container: false` to prevent UIkit from moving DOM nodes. When using `useUIKit('modal', ...)` directly (without `<Modal>`), set `container: false` and wrap the JSX in `createPortal`. Never use UIkit's `container: '#root'` — it moves DOM nodes via native `appendChild`, which breaks React's reconciliation and causes `removeChild` crashes when components unmount.
...
47. **Margin-Free Animated Wrappers**: Never apply CSS margins (`uk-margin-*`) directly on a `<motion.div>` that animates `height` to/from 0 (e.g., `expandCollapseProps`). Margins live outside the border box and are **not** clipped by `overflow: hidden` — they persist at full size until the element is unmounted, causing a visible "jump" at the end of exit animations. Instead, wrap the content in a child `<div>` that carries the margin class so it gets clipped along with the content during the height transition.
48. **Adaptive Density via Tokens**: Govern how content fits on narrow screens through the global density tokens (ADR-003 / §X), not per-component media queries. The grid gap ("breathing room") is the canonical `--density-gap` token mapped to UIkit's gutter scale (`$global-medium-gutter` → `$global-gutter` → `$global-small-gutter` = 40/30/15px) and applied via `Grid gap="adaptive"` — generous on desktop, tight on mobile. Keep deliberately-dense card-interior values (`--density-pad`/`--density-stack`) as documented literals only because UIkit's spacing scale is too coarse for them.
49. **Build Integrity**: After introducing any feature or change, the build must successfully complete (`npm run build`). This ensures that type safety is maintained and the application is always in a deployable state.
