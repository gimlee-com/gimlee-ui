...
import type { AddRatingResponseRequestDto } from '../../types/ratings';

interface ResponseFormProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: AddRatingResponseRequestDto) => void;
...
export default function ResponseForm({
  ...
  const handleFormSubmit = (data: ResponseFormValues) => {
    ...
    <div ref={modalRef} className="uk-modal-container">
      <div className="uk-modal-dialog">
        ...
        <div className="uk-modal-header">
          <h2 className="uk-modal-title">{t('reviews.responseForm.title')}</h2>
        </div>
        <Form onSubmit={handleSubmit(handleFormSubmit)}>
          <div className="uk-modal-body">
            <p className="uk-text-meta">{t('reviews.responseForm.description')}</p>

            <motion.div layout className="uk-margin">
              <FormLabel>{t('reviews.responseForm.bodyLabel')}</FormLabel>
              <FormControls>
                <Controller
                  name="body"
                  control={control}
                  rules={{
                    required: t('reviews.responseForm.bodyRequired'),
                    maxLength: {
                      value: 5000,
                      message: t('reviews.responseForm.bodyTooLong'),
                    },
                  }}
                  render={({ field }) => (
                    <MarkdownEditor
          ...
          <div className="uk-modal-footer uk-text-right">
            ...
            >
              {isSubmitting ? t('common.loading') : t('reviews.responseForm.submit')}
            </button>
          </div>
        </Form>
      </div>
